...
class RecipeFinderService {
    ...
    private def filterItemsPastUseByDate(fridgeItems)
    {
    	...
    	for(FridgeItem fridgeItem : fridgeItems)
    	{
    		...
    		{
    			useableItemsMap.put(fridgeItem.item, [fridgeItem.amount, fridgeItem.useBy])
    		}
    	}
    ...
    private def findCookableRecipes(recipesAvailable, Map useableItemsMap)
    {
    	...
    	for(Recipe recipe : recipesAvailable)
    	{
    		...
    		{
    			if(useableItemsMap.containsKey(ingredient.item))
    			{
    				def amountInFridge = useableItemsMap.get(ingredient.item)[0]
    				...
    				{
    					def itemUseByDate = useableItemsMap.get(ingredient.item)[1]
